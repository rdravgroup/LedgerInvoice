...
export class CreateinvoiceComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.isedit = this.editinvoiceno != null;
    if (this.isedit) this.pagetitle = 'Edit Invoice';
  ...
  initializeForm() {
    ...
    this.invoiceform = this.builder.group({
      ...
      invoiceDate: this.builder.control(today, Validators.required),
  ...
  addnewproduct() {
    ...
    if (this.invoiceDisplayNumberMode !== 'auto' && (!num || !String(num).trim())) {
      this.alert.warning('Enter an invoice number first','Validation');
      return;
    }
    ...
    this.companyService.getCompanyById(companyId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (company: any) => {
        ...
        if (!this.isedit && this.invoiceDisplayNumberMode === 'auto') {
          ...
          invoiceNumberControl?.disable({ emitEvent: false });
          invoiceNumberControl?.setValue('', { emitEvent: false });
        } else if (!this.isedit) {
          invoiceNumberControl?.enable({ emitEvent: false });
        ...
        if (!this.isedit) {
          ...
          if (this.invoiceDisplayNumberMode === 'manual') {
            this.generateInvoiceDisplayNumber(companyId);
          }
        }
      ...
      error: () => {
        ...
        this.invoiceDisplayNumberMode = 'manual';
        this.invoiceform.get('invoiceNumber')?.enable({ emitEvent: false });
        this.invoiceform.get('invoiceNumber')?.setValidators([Validators.required]);
        this.invoiceform.get('invoiceNumber')?.updateValueAndValidity({ emitEvent: false });
  ...
  private generateInvoiceDisplayNumber(companyId: string): void {
    if (this.isedit || !companyId) return;
    const invoiceDate = this.invoiceform.get('invoiceDate')?.value;
    if (!(invoiceDate instanceof Date)) return;
    this.service.GenerateDisplayInvoiceNumber(companyId, invoiceDate).subscribe({
      next: (response: any) => {
        const generated = response?.displayInvNumber || response?.DisplayInvNumber;
        if (!generated || this.isedit) return;
        const control = this.invoiceform.get('invoiceNumber');
        control?.setValue(generated, { emitEvent: false });
        if (this.invoiceDisplayNumberMode === 'auto') control?.disable({ emitEvent: false });
      },
      error: () => {
        this.alert.warning('Invoice number could not be generated. You can enter it manually.', 'Invoice number');
      }
    });
  }
  ...
  GetCustomers(selectCustomer?: any) {
    const companyId = this.invoiceform.get('companyId')?.value || this.getEffectiveCompanyId();
    this.service.EnsureCashCustomer(companyId).pipe(
      ...
      next: (res: any) => {
        ...
        if (!this.isedit && !this.invoiceform.get('customerId')?.value) {
          ...
          const cashCustomer = this.mastercustomer.find(customer =>
            String(customer.uniqueKeyID).toUpperCase() === cashCode);
          if (cashCustomer) {
            this.invoiceform.get('customerId')?.setValue(cashCustomer.uniqueKeyID);
            this.customerchange(cashCustomer.uniqueKeyID);
          }
        }
        if (!this.isedit && !this.invoiceform.get('customerId')?.value) {
          this.customerDropdownOpen = false;
